...
def load_best_basic9_params(outputs_dir: Path) -> tuple[dict | None, str | None]:
    ...
    return params, metrics_path.name
...
def main() -> None:
    ...
    # Use all rows with valid datetime + target from the provided train+miss_train snapshot.
    ...
    # Expanding-window rolling 3-fold:
    # split into 4 contiguous blocks; use block 2/3/4 as validation.
    # fold0: train=block1, valid=block2
    # fold1: train=block1+2, valid=block3
    # fold2: train=block1+2+3, valid=block4
    block = n // 4
    if block < 1:
        raise ValueError("Block size is zero; cannot build rolling folds.")
    ...
    for fold_id, train_idx, valid_idx in folds:
        ...
        model_name = f"xgboost_basic9_time_rolling_3fold_fold{fold_id}.json"
        model.save_model(str(models_dir / model_name))
        fold_model_files.append(model_name)
    ...
    # Full training model on the same dataset
    ...
    full_model_name = "xgboost_basic9_time_rolling_3fold_full.json"
    full_model.save_model(str(models_dir / full_model_name))

    oof = work_df[["datetime", "source", TARGET]].copy()
    oof["pred_count"] = oof_pred
    oof_path = outputs_dir / "oof_xgboost_basic9_time_rolling_3fold.csv"
    oof.to_csv(oof_path, index=False)

    payload = {
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "input_data": data_path.name,
        "feature_strategy": "base9 (season,holiday,workingday,weather,temp,atemp,humidity,windspeed,hour)",
        "features": FEATURES,
        "target": TARGET,
        ...
        "model_paths": fold_model_files + [full_model_name],
        "oof_path": oof_path.name,
    }

    metrics_path = outputs_dir / "xgboost_basic9_time_rolling_3fold_metrics.json"
    metrics_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")

    report_path = reports_dir / "xgboost_basic9_time_rolling_3fold_report.md"
    lines = [
        "# XGBoost Basic9 Time Rolling 3-Fold Report",
        "",
