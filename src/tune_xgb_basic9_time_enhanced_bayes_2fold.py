...
def main() -> None:
    ...
    for fold in folds:
        ...
        model.save_model(str(models_dir / f"xgboost_basic9_time_enhanced_bayes_2fold_fold{fold}.json"))
    ...
    full_model_path = models_dir / "xgboost_basic9_time_enhanced_bayes_2fold_full.json"
    full_model.save_model(str(full_model_path))

    oof = df[["datetime", "source", TARGET, FOLD_COL]].copy()
    oof["pred_count"] = oof_pred
    oof_path = outputs_dir / "oof_xgboost_basic9_time_enhanced_bayes_2fold.csv"
    oof.to_csv(oof_path, index=False)

    trials_df = study.trials_dataframe(
        attrs=("number", "value", "state", "params", "datetime_start", "datetime_complete")
    )
    tuning_path = outputs_dir / "xgboost_basic9_time_enhanced_bayes_2fold_tuning_history.csv"
    trials_df.to_csv(tuning_path, index=False)

    best_params_path = outputs_dir / "xgboost_basic9_time_enhanced_bayes_2fold_best_params.json"
    ...
    payload = {
        ...
        "feature_strategy": "base9 + datetime_derived_time_features + bayesian_hpo",
        ...
        "model_paths": [
            "xgboost_basic9_time_enhanced_bayes_2fold_fold0.json",
            "xgboost_basic9_time_enhanced_bayes_2fold_fold1.json",
    ...
    metrics_path = outputs_dir / "xgboost_basic9_time_enhanced_bayes_2fold_metrics.json"
    metrics_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")

    report_path = reports_dir / "xgboost_basic9_time_enhanced_bayes_2fold_report.md"
    report_path.write_text(
        "\n".join(
            [
                "# XGBoost Basic9 Time-Enhanced Bayesian 2-Fold Report",
                "",
